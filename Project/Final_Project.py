...
def login_signup_systems():
    ...
    def verifyUser(view, account_entry, password_entry):
        print(account_entry.get(), password_entry.get())
        print("Log in...")
    # store the password and account in a variable
        account = account_entry.get()
        password = password_entry.get()
        try:
            ...
            result_label.config(text="Successfully loggedd in!")
        except Exception as e:
            print(f"Logged in failed: {e}")
            result_label["text"]="logged in failed!"
    ...
    login_B=Button(system,text="login",command=lambda:verifyUser(system,account_entry, password_entry))
    signup_B=Button(system,text="signup",command=lambda:addUser(system,account_entry, password_entry))
...
def reciept():
    ...
    if a!="":
        ...
        if result=="yes":
            tempContent = "Hi user, you just buy a total of {}".format(subtotal1+subtotal2+subtotal3+subtotal4)
